...
export function lastInternalTimestampMs(raw: string): number | null {
 ...
 */
export function ageBasisMs(rawOrNull: string | null, mtimeMs: number): number {
  if (rawOrNull === null) return mtimeMs;
  const internal = lastInternalTimestampMs(rawOrNull);
  if (internal === null) return mtimeMs;
