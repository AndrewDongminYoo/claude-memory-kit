...
function temporaryBase(options) {
    ...
    if (isWithin(claudeRoot, base) ||
        isWithin(claudeRoot, resolvedBase) ||
        isWithin(resolvedClaudeRoot, base) ||
        isWithin(resolvedClaudeRoot, resolvedBase)) {
        throw new Error("temporary fixture base must not be inside the Claude configuration root");
    }
    return resolvedBase;
}
function copyDistribution(pluginRoot, copyRoot) {
...
function runNode(cwd, configRoot, script, args, env = process.env) {
...
function validateMetadata(copyRoot, configRoot) {
    const result = childProcess.spawnSync("claude", ["plugin", "validate", "--strict", "."], {
        cwd: copyRoot,
        encoding: "utf8",
...
export function verifyPluginCopy(pluginRoot, options = {}) {
    const temporaryRoot = fs.mkdtempSync(path.join(temporaryBase(options), "cmk-plugin-"));
    ...
    try {
        ...
        const missingScope = runNode(copyRoot, configRoot, "dist/scan-cold.js", [], { ...process.env, CMK_SCOPE_SLUG_PREFIXES: "" });
        if (missingScope.status === 0 ||
        ...
        fs.symlinkSync(outsideProjectsDir, path.join(symlinkConfigRoot, "projects"));
        const symlinkProjectsScan = runNode(copyRoot, symlinkConfigRoot, "dist/scan-cold.js", [], verificationEnv);
        if (symlinkProjectsScan.status === 0 ||
            !symlinkProjectsScan.stderr.includes("projects directory is a symbolic link")) {
            throw new Error("compiled scan-cold did not reject a symlinked projects root");
        }
        const symlinkProjectsScore = runNode(copyRoot, symlinkConfigRoot, "dist/score-prefilter.js", [
            path.join(symlinkConfigRoot, "projects", "verification-project", "session.jsonl"),
        ], verificationEnv);
        if (symlinkProjectsScore.status === 0 ||
            !symlinkProjectsScore.stderr.includes("projects directory is a symbolic link")) {
            throw new Error("compiled score-prefilter did not reject a symlinked projects root");
        }
        const invalidColdDays = runNode(copyRoot, configRoot, "dist/scan-cold.js", [], { ...verificationEnv, COLD_DAYS: "not-a-number" });
        if (invalidColdDays.status === 0 ||
            !invalidColdDays.stderr.includes("invalid COLD_DAYS")) {
            throw new Error("compiled scan-cold did not reject invalid COLD_DAYS");
        }
        const transcript = path.join(configRoot, "projects", "verification-project", "session.jsonl");
        ...
        const candidates = JSON.parse(coldScan.stdout);
        ...
        const symlinkedTranscript = path.join(configRoot, "projects", "verification-project", "linked.jsonl");
        fs.writeFileSync(outsideTranscript, "{ invalid\n");
        fs.symlinkSync(outsideTranscript, symlinkedTranscript);
        const symlinkedScore = runNode(copyRoot, configRoot, "dist/score-prefilter.js", [symlinkedTranscript], verificationEnv);
        if (symlinkedScore.status === 0 ||
