 ...
 */
export function parseScopeSlugPrefixes(value = process.env[SCOPE_SLUG_PREFIXES_ENV]) {
    const prefixes = (value ?? "")
        .split(",")
        .map((prefix) => prefix.trim())
        .filter(Boolean);
    if (prefixes.length === 0) {
        throw new Error(`${SCOPE_SLUG_PREFIXES_ENV} is required and must not be empty`);
    }
    return [...new Set(prefixes)];
}
export function isSlugInScope(slug, prefixes) {
    return prefixes.some((prefix) => slug.startsWith(prefix));
}
export function isSingleSegmentSlug(slug) {
...
export function assertSingleSegmentSlug(slug) {
    if (!isSingleSegmentSlug(slug)) {
        throw new Error(`slug ${JSON.stringify(slug)} is not a single project slug`);
    }
}
export function assertSlugInScope(slug, prefixes) {
    assertSingleSegmentSlug(slug);
    if (!isSlugInScope(slug, prefixes)) {
        throw new Error(`slug ${JSON.stringify(slug)} is outside ${SCOPE_SLUG_PREFIXES_ENV}`);
    }
}
/** Reject a path that is not a direct projects/<slug>/*.jsonl transcript. */
export function assertDirectTranscriptPath(transcriptPath, slug, configuredProjectsDir) {
    assertSingleSegmentSlug(slug);
    ...
    if (path.dirname(slugDir) !== projectsRoot ||
        path.dirname(candidate) !== slugDir ||
        path.extname(candidate) !== ".jsonl") {
        throw new Error(`transcript path ${JSON.stringify(transcriptPath)} is not a direct transcript for slug ${JSON.stringify(slug)}`);
    }
}
function assertCurrentSafeTranscriptPath(transcriptPath, slug, configuredProjectsDir, openedStat, expectedProjectsStat) {
    ...
    if (expectedProjectsStat &&
        (projectsStat.dev !== expectedProjectsStat.dev ||
            projectsStat.ino !== expectedProjectsStat.ino)) {
        throw new Error("projects directory changed after the transcript was opened");
    }
    ...
    if (slugStat.isSymbolicLink() || transcriptStat.isSymbolicLink()) {
        throw new Error("transcript path or project slug directory is a symbolic link");
    }
    if (!slugStat.isDirectory() || !transcriptStat.isFile()) {
        throw new Error("transcript path is not a regular file in a project directory");
    }
    if (openedStat &&
    ...
    if (path.dirname(resolvedSlugDir) !== resolvedProjectsRoot ||
...
export function assertSafeTranscriptFile(transcriptPath, slug, configuredProjectsDir) {
...
export function readSafeTranscriptFile(transcriptPath, slug, configuredProjectsDir) {
    const descriptor = openSafeTranscriptFile(transcriptPath, slug, configuredProjectsDir);
    try {
